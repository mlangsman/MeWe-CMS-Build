 ...
 * @package 	Solspace:User
 * @author		Solspace DevTeam
 * @filesource 	./system/expressionengine/third_party/user/mod.user.php
 */
...
class User extends Module_builder_user
{
		...
		foreach ($dates as $val)
		{
			...
			if (preg_match_all("/".LD.$val."\s+format=([\"'])([^\\1]*?)\\1".RD."/s", ee()->TMPL->tagdata, $matches))
			{
				...
				for ($j = 0; $j < count($matches[0]); $j++)
				{
					...
					$date_vars[$val][$matches[0][$j]] = ee()->localize->fetch_date_params($matches[2][$j]);
				}
			}
		}
				...
				foreach($date_vars as $type => $matches)
				{
					...
					if (APP_VER < 2.0)
					{
						foreach ( $matches[$key] as $code )
						{
							$val	= str_replace( 
									$code, 
									ee()->localize->convert_timestamp($code, $row[$type], TRUE ), 
									$val);
						}						
					}
					else
	...
	public function edit()
	{
		...
		if ( ee()->session->userdata['member_id'] == $this->member_id OR ee()->session->userdata['group_id'] == 1 OR 
		   ( ee()->TMPL->fetch_param('group_id') !== FALSE AND in_array( ee()->session->userdata['group_id'], preg_split( "/,|\|/", ee()->TMPL->fetch_param('group_id') ) ) ) )
		{
		...
		{
			$edata = ee()->extensions->universal_call( 'user_edit_start', $this->member_id );
			if (ee()->extensions->end_script === TRUE) return;
		}
	...
	public function edit_profile()
	{
		...
		if ( ee()->session->userdata['group_id'] == 1 OR 
			( $this->_param('group_id') AND 
			  in_array( ee()->session->userdata['group_id'], preg_split( "/,|\|/", $this->_param('group_id') ) ) ) 
		 )
		{
			$admin	= TRUE;
		}
		...
		if ( ee()->session->userdata['member_id'] != $member_id AND $admin === FALSE )
		{
			return $this->_output_error('general', array(ee()->lang->line('not_authorized')));
		}
		...
		*/
		...
		if ( ee()->input->post('username') AND 
			 ee()->session->ban_check('username', ee()->input->post('username')) )
		{
			return $this->_output_error('general', array(ee()->lang->line('prohibited_username')));
		}
		
		if ($this->_param('exclude_username') != '' AND 
			in_array(ee()->input->post('username'), explode('|', $this->_param('exclude_username'))))
		{
			return $this->_output_error('general', array(ee()->lang->line('prohibited_username')));
		}
		...
		{		
			$line = str_replace(
				...
				ee()->lang->line('password_lockout_in_effect')
			);		
		...
		if (ee()->input->get_post('password') !== FALSE AND 
			...
			ee()->input->get_post('current_password') != ee()->input->get_post('password') AND 
			$member_id == ee()->session->userdata['member_id'])
		{				
		...
		if ($admin === FALSE && $this->check_yes($this->_param('password_required')))
		{
			if (ee()->session->check_password_lockout() === TRUE)
			{		
				$line = str_replace(
			...
			//2.2.x+ needs auth+salt checking
			else
			{				
				$passwd = ee()->auth->hash_password(ee()->input->get_post('current_password'), $query->row('salt'));
						...
						if (ee()->functions->hash(stripslashes($_POST['current_password'])) != $query->row('password'))
						{
							...
							if ($check == 'email')
							{
								return $this->_output_error( 
									'general', 
									array( ee()->lang->line( 'current_password_required_email' ) ) 
								);
							}
							else
							{
								return $this->_output_error( 
									'general', 
									array( ee()->lang->line( 'invalid_password' ) ) 
								);
							}
						}
					}
		...
		*/
		if (ee()->extensions->active_hook('user_edit_validate') === TRUE)
		{
			...
			{
				ee()->validate->errors = array_merge(ee()->validate->errors, $errors);
			}		
		}			
		/*
		/* -------------------------------------------*/
			...
			ee()->config->item('forum_is_installed') == "y" )
		{
			ee()->db->query(
				"UPDATE exp_forums 
				 SET 	forum_last_post_author = '".ee()->db->escape_str($_POST['screen_name'])."' 
				 WHERE 	forum_last_post_author_id = '".$member_id."'"
			);
		}
			 ...
			 ee()->input->get_post('password') != '' )
		{
			...
			{
				ee()->auth->update_password($member_id, stripslashes($_POST['password']));
			}
		}
		...
		elseif(ee()->input->post('group_id') !== FALSE AND ctype_digit(ee()->input->post('group_id')) AND $this->_param('allowed_groups') !== FALSE)
		{
		...
		*/
		...
		if ($update_admin_email === TRUE AND trim($member_update_admin_notification_emails) != '')
		{
	...
	{	
		...
		if (ee()->config->item('allow_member_registration') == 'n')
		{
			return $this->_output_error(
				'general', 
				array(ee()->lang->line('registration_not_enabled'))
			);
		}
		...
		{ 
				 ...
				 ee()->session->userdata['can_admin_members'] !== 'y'))
			{
				return $this->_output_error(
					'general', 
					array(ee()->lang->line('mbr_you_are_registered'))
				);
			}
		}
		...
		if (APP_VER >= '2.2.0')
		{
			...
			if ( ! ee()->auth->check_require_ip())
			{
				return $this->_output_error(
					'general', 
					array(ee()->lang->line('not_authorized'))
				);
			}
		}
		...
		if (ee()->session->ban_check('username', $_POST['username']))
		{
			return $this->_output_error(
				'general', 
				array(ee()->lang->line('prohibited_username'))
			);
		}
	   ...
	   /** --------------------------------------------*/
			 ...
			 $this->_param('key_email_match') == 'yes' )
		{
			...
			if ( ! ee()->input->post('key') )
			{
				return $this->_output_error(
					'submission', 
					array(ee()->lang->line('key_required'))
				);
			}
				...
				! ee()->input->get_post('email') )
			{
				return $this->_output_error(
					'submission', 
					array(ee()->lang->line('key_email_match_required'))
				);
			}
			...
			if ( $query->num_rows() == 0 )
			{
				...
				return $this->_output_error(
					'submission', 
					array( str_replace( "%s", $exp, ee()->lang->line('key_incorrect')))
				);
			}
		...
		if ( $query->row('count') == 0 )
		{
			return $this->_output_error(
				'submission', 
				array(ee()->lang->line('invalid_member_group'))
			);
		}
				 ...
				 ee()->session->userdata('member_id') == 0))
			{
				...
				else
				{
					...
					if ($query->row('count') == 0)
					{
						return $this->_output_error(
							'submission', 
							array(ee()->lang->line('captcha_incorrect'))
						);
					}
		...
		elseif($this->_param('admin_register') != 'yes')
		{
			...
			{			
				...
				ee()->functions->set_cookie(
					ee()->session->c_uniqueid , 
					$this->insert_data['unique_id'], 
					$expire
				); 
				
				ee()->functions->set_cookie(
					ee()->session->c_password , 
		...
		if (preg_match_all("/".LD."changed(.*?)".RD."(.*?)".LD.'\/changed'.RD."/s", $message, $matches))
		{
			...
			for ($j = 0; $j < count($matches['0']); $j++)
			{
				...
				foreach($changed as $key => $value)
				{
					...
					{
						if (ee()->lang->line($key) != FALSE AND ee()->lang->line($key) != '')
						{
	...
	public function activate_member()
	{
		...
		{                       
			ee()->output->show_message(array(	
				'title' 	=> ee()->lang->line('mbr_activation'),
	...
	{	
		...
		if (APP_VER < '2.2.0')
		{
			...
			if ($query->row('password') != $password)
			{
				...
				ee()->config->set_item(
					'encryption_type', 
					(ee()->config->item('encryption_type') == 'md5') ? 'sha1' : 'md5'
				);
        ...
        foreach($possible AS $var)
        {
        	if (ee()->input->get_post($var) !== FALSE)
        	{
				...
				foreach ( $email as $e )
				{
					if ( ! valid_email($e))
					{
						return $this->_output_error('general', array(ee()->lang->line('invalid_email_address')));
					}
				}
        	}
        }
	...
	private function _remove_image($type, $member_id, $admin)
	{
		...
		if ($admin === FALSE AND ($this->check_yes($this->_param('password_required')) OR $this->_param('password_required') == 'yes'))
		{
			...
			if (APP_VER >= '2.2.0' AND
				! ee()->auth->authenticate_id($member_id, $_POST['current_password']))
			{
				return $this->_output_error( 'general', array( ee()->lang->line( 'invalid_password' ) ) );
			}
