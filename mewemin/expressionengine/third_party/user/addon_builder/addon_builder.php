...
EOT;
	...
	{	
		...
		)
		{
			$query = ee()->db->query(
				preg_replace(
					"/SELECT (DISTINCT)?(.*?)\s+FROM\s+/is",
					'SELECT \\1 COUNT(*) AS count FROM ',
					$input_data['sql'],
					1
				)
			);
		...
		// $return_data['current_page'] += $input_data['offset'];	// Why oh why are you treating offset like a page offset instead of just a row offset? MK confused.
